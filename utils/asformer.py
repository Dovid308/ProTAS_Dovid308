...
class AttLayer(nn.Module):
    def __init__(self, q_dim, k_dim, v_dim, r1, r2, r3, bl, stage, att_type,causal=False): # r1 = r2 #modificato per causalità
    ...
    #        #window_mask[:, :, i:i+self.bl] = 1 dicono loro di cambiare qua
    #        window_mask[:, i, i:i+self.bl] = 1
    #    return window_mask.to(device)


    def construct_window_mask(self, causal=False):
        """
        Mask per sliding window attention.
        Shape: (1, bl, bl + 2 * (bl // 2))
        - causal=False: ogni query vede tutta la finestra locale
        - causal=True: ogni query vede solo passato + se stessa
        """
        window_mask = torch.zeros((1, self.bl, self.bl + 2 * (self.bl // 2)))
        half = self.bl // 2
        ...
        for i in range(self.bl):
            ...
            if causal:
                center = half + i
                window_mask[:, i, start:center + 1] = 1
            else:
...
class AttModule(nn.Module):
    def __init__(self, dilation, in_channels, out_channels, r1, r2, att_type, stage, alpha, causal=False):
        super(AttModule, self).__init__()
        self.feed_forward = ConvFeedForward(dilation, in_channels, out_channels)
        self.instance_norm = nn.InstanceNorm1d(in_channels, track_running_stats=False)
        self.att_layer = AttLayer(in_channels, in_channels, out_channels, r1, r1, r2, dilation, att_type=att_type, stage=stage, causal=causal) # dilation
...
class Encoder(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes,
                 channel_masking_rate, att_type, alpha, causal=False):
